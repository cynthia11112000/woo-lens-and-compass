...
def _call_chat_day_enrich(client, day_key: str, messages: list[dict], chat_prompt: str = _CHAT_DAY_PROMPT) -> dict:
...
def _build_chat_day_events(client, grouped: dict[str, list[dict]], chat_prompt: str = _CHAT_DAY_PROMPT) -> list[dict]:
    ...
    for idx, day_key in enumerate(day_keys, start=1):
        ...
        enrich = _call_chat_day_enrich(client, day_key, day_messages, chat_prompt)
...
def _call_enrich(client, batch: list[dict], batch_num: int, total_batches: int, system_prompt: str = _ENRICH_SYSTEM) -> list[dict]:
...
def enrich_events(docs: dict, api_key: str, language: str = "nl") -> list[dict]:
    """
    ...
    Args:
        ...
        language: Output language — 'nl' (default), 'en', or 'pap'.
    ...
    _lang_suffix = _ENRICH_LANG_SUFFIX.get(language, "")
    _enrich_system = _ENRICH_SYSTEM + (f"\n\n{_lang_suffix}" if _lang_suffix else "")
    _chat_prompt   = _CHAT_DAY_PROMPT + (f"\n\n{_lang_suffix}" if _lang_suffix else "")
    ...
    if emails:
        ...
        for idx, batch in enumerate(batches, start=1):
            ...
            events = _call_enrich(client, batch, idx, total, system_prompt=_enrich_system)
    ...
    if chat_messages_by_day:
        ...
        chat_events = _build_chat_day_events(client, chat_messages_by_day, chat_prompt=_chat_prompt)
